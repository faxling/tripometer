...
static void _error_exit(j_common_ptr cinfo)
{
  ...
  g_set_error(myerr->error, MAEP_LOADER_ERROR, MAEP_LOADER_ERROR_JPEG, "%s", msg);

  /* Return control to the setjmp point */
  longjmp(myerr->setjmp_buffer, 1);
}

static gboolean img_loader_jpeg(struct jpeg_decompress_struct* cinfo, cairo_surface_t** surf,
                                GError** error)
{
  ...
  {
    g_set_error(error, MAEP_LOADER_ERROR, MAEP_LOADER_ERROR_JPEG_HEADER, "wrong JPEG header");
  ...
  {
    g_set_error(error, MAEP_LOADER_ERROR, MAEP_LOADER_ERROR_JPEG_DECOMPRESS, "decompress error");
    jpeg_destroy_decompress(cinfo);
    return FALSE;
  }

  /* Image dimensions. */
  *surf = cairo_image_surface_create(CAIRO_FORMAT_RGB24, cinfo->output_width, cinfo->output_height);
  ...
  {
    g_set_error(error, MAEP_LOADER_ERROR, MAEP_LOADER_ERROR_JPEG_SCANLINES, "scanlines error");
    jpeg_destroy_decompress(cinfo);
...
cairo_surface_t* maep_loader_jpeg_from_file(const char* filename, GError** error)
{
  ...
  {
    g_set_error(error, MAEP_LOADER_ERROR, MAEP_LOADER_ERROR_FILE, "can't open %s", filename);
    return NULL;
  }
