 ...
 */
static gchar *replace_string(const gchar *src, const gchar *from,
                             const gchar *to);
...
#if USE_LIBSOUP22
static void osm_gps_map_tile_download_complete(SoupMessage *msg,
                                               gpointer user_data);
#else
static void osm_gps_map_tile_download_complete(SoupSession *session,
                                               SoupMessage *msg,
                                               gpointer user_data);
#endif
static void osm_gps_map_download_tile(OsmGpsMap *map, int zoom, int x, int y,
                                      gboolean redraw);
static void osm_gps_map_load_tile(OsmGpsMap *map, int zoom, int x, int y,
...
static void map_convert_coords_to_quadtree_string(gint x, gint y,
                                                  gint zoomlevel, gchar *buffer,
                                                  const gchar initial,
...
static gchar *get_tile_uri(const gchar *uri, int uri_format, int max_zoom,
...
gchar *osm_gps_map_source_get_tile_uri(OsmGpsMapSource_t source, int zoom,
                                       int x, int y) {
...
static float get_distance(float lat1, float lon1, float lat2, float lon2) {
  float aob = acos(
      CLAMP(cos(lat1) * cos(lat2) * cos(lon2 - lon1) + sin(lat1) * sin(lat2),
            -1.f, +1.f));
    ...
    while (tmp != NULL) {
      ...
      if (pNode->m_nId == -1) {
        ...
        if (tLast.rlat == 0)
          return 0;
        float dist =
            get_distance(deg2rad(la), deg2rad(lo), tLast.rlat, tLast.rlon);
...
void osm_gps_map_layer_changed(OsmGpsMap *map,
                               G_GNUC_UNUSED OsmGpsMapLayer *layer) {
  OsmGpsMapPrivate *priv;

  g_return_if_fail(OSM_IS_GPS_MAP(map));
  priv = map->priv;
  if (!priv->idle_map_redraw)
    priv->idle_map_redraw =
        g_idle_add((GSourceFunc)osm_gps_map_idle_redraw, map);
}
...
static void osm_gps_map_print_images(OsmGpsMap *map) {
  ...
  cairo_select_font_face(priv->cr, "Purisa", CAIRO_FONT_SLANT_NORMAL,
  ...
  for (list = priv->images; list != NULL; list = list->next) {
    ...
    */
    ...
    cairo_set_source_surface(priv->cr, im->image, x - im->xoffset,
                             y - im->yoffset);
    cairo_paint(priv->cr);
    ...
    if (im->image == priv->cr_markedImage) {
      x = x - im->xoffset;
      y = y - im->yoffset;
g_message("markedImage %dx%d",x,y);
      cairo_move_to(priv->cr, x, y);
      cairo_line_to(priv->cr, x + im->w, y);
      cairo_line_to(priv->cr, x + im->w, y + im->h);
      cairo_line_to(priv->cr, x, y + im->h);
...
static void osm_gps_map_draw_gps_point(OsmGpsMap *map) {
  ...
  if (priv->gps_valid) {
    ...
    if (r > 0) {
      ...
      pat = cairo_pattern_create_radial(x - (r / 5), y - (r / 5), (r / 5), x, y,
                                        r);
...
static void osm_gps_map_blit_surface(OsmGpsMap *map, cairo_surface_t *cr_surf,
                                     int offset_x, int offset_y, int modulo,
  ...
  cairo_save(priv->cr);
  cairo_translate(priv->cr, offset_x - area_x * modulo,
                  offset_y - area_y * modulo);
  ...
  if (SOUP_STATUS_IS_SUCCESSFUL(msg->status_code)) {
    ...
    if (dl->redraw) {
      if (priv->cache_dir && file_saved)
        ...
        cr_surf = osm_gps_map_from_mem(
            (const unsigned char *)MSG_RESPONSE_BODY(msg),
...
static void osm_gps_map_download_tile(OsmGpsMap *map, int zoom, int x, int y,
  ...
  dl->uri = get_tile_uri(priv->repo_uri, priv->uri_format, priv->max_zoom, zoom,
                         x, y);
  ...
  if (g_hash_table_lookup_extended(priv->tile_queue, dl->uri, NULL, NULL) ||
    ...
    dl->folder =
        g_strdup_printf("%s%c%d%c%d%c", priv->cache_dir, G_DIR_SEPARATOR, zoom,
                        G_DIR_SEPARATOR, x, G_DIR_SEPARATOR);
    dl->filename = g_strdup_printf("%s%c%d%c%d%c%d.%s", priv->cache_dir,
                                   G_DIR_SEPARATOR, zoom, G_DIR_SEPARATOR, x,
                                   G_DIR_SEPARATOR, y, priv->image_format);
    dl->map = map;
    dl->redraw = redraw;
    ...
    if (msg) {
      ...
      soup_session_queue_message(priv->soup_session, msg,
                                 osm_gps_map_tile_download_complete, dl);
    } else {
...
gchar *get_cached_file(const gchar *cache_dir, const gchar *format, int zoom,
                       int x, int y) {
  gchar *filename;

  filename =
      g_strdup_printf("%s%c%d%c%d%c%d.%s", cache_dir, G_DIR_SEPARATOR, zoom,
                      G_DIR_SEPARATOR, x, G_DIR_SEPARATOR, y, format);
...
gchar *osm_gps_map_source_get_cached_file(OsmGpsMapSource_t source,
                                          const gchar *cache_dir, int zoom,
                                          int x, int y) {
  return get_cached_file(cache_dir, osm_gps_map_source_get_image_format(source),
                         zoom, x, y);
}

static OsmCachedTile *osm_gps_map_load_cached_tile(OsmGpsMap *map,
...
static OsmCachedTile *osm_gps_map_find_bigger_tile(OsmGpsMap *map, int zoom,
                                                   int x, int y,
                                                   int *zoom_found) {
  OsmCachedTile *tile;
  gchar *filename;
  ...
  filename = get_cached_file(map->priv->cache_dir, map->priv->image_format,
...
static OsmCachedTile *osm_gps_map_render_missing_tile(OsmGpsMap *map, int zoom,
                                                      int x, int y, int *modulo,
                                                      int *area_x,
                                                      int *area_y) {
  /* maybe TODO: render from downscaled tiles, if the following fails */
  /* g_message("look for upscaled at %dx%d.", x, y); */
  return osm_gps_map_render_missing_tile_upscaled(map, zoom, x, y, modulo,
                                                  area_x, area_y);
}
...
static gboolean osm_gps_map_tile_age_exceeded(char *filename, guint period) {
  struct stat buf;
...
static void osm_gps_map_load_tile(OsmGpsMap *map, int zoom, int x, int y,
  ...
  if (!tile || needs_refresh) {
    ...
    if (!needs_refresh &&
       ...
       * levels */
      tile = osm_gps_map_render_missing_tile(map, zoom, x, y, &modulo, &area_x,
                                             &area_y);
      if (tile) {
        /* g_message("Tile not found, upscaling."); */
        osm_gps_map_blit_surface(map, tile->cr_surf, offset_x, offset_y, modulo,
                                 area_x, area_y);
      }
      /* else */
      /* { */
...
static void osm_gps_map_fill_tiles_pixel(OsmGpsMap *map) {
  ...
  fmap_x =
      priv->map_x + 0.5 * priv->viewport_width * (1. - 1. / priv->map_factor);
  fmap_y =
      priv->map_y + 0.5 * priv->viewport_height * (1. - 1. / priv->map_factor);
  ...
  offset_xn =
      offset_x + 0.5 * priv->viewport_width * (1.5 - 1. / priv->map_factor);
  offset_yn =
      offset_y + 0.5 * priv->viewport_height * (1.5 - 1. / priv->map_factor);

  tiles_nx =
      (priv->viewport_width / priv->map_factor - offset_x) / tilesize + 1;
  tiles_ny =
      (priv->viewport_height / priv->map_factor - offset_y) / tilesize + 1;
  ...
  for (i = tile_x0; i < (tile_x0 + tiles_nx); i++) {
    for (j = tile_y0; j < (tile_y0 + tiles_ny); j++) {
      if (j < 0 || i < 0 || i >= exp(priv->map_zoom * M_LN2) ||
          j >= exp(priv->map_zoom * M_LN2)) {
...
void osm_gps_map_get_tile_xy_at(OsmGpsMap *map, float lat, float lon, int *zoom,
                                int *x, int *y) {
  int tilesize;
  ...
  *x = (int)floor(lon2pixel(map->priv->map_zoom, deg2rad(lon)) /
                  (float)tilesize);
  *y = (int)floor(lat2pixel(map->priv->map_zoom, deg2rad(lat)) /
                  (float)tilesize);
}
...
static void draw_startpoint(OsmGpsMapPrivate *priv, MaepGeodataTrackIter iter) {
  ...
  cairo_set_source_rgba(priv->cr, 0xFF / 255.0, 0x14 / 255.0, 0x93 / 255.0,
                        0.6);
  cairo_fill_preserve(priv->cr);
...
static void osm_gps_map_print_track(OsmGpsMapPrivate *priv, MaepGeodata *track,
                                    int lw, int *max_x, int *min_x, int *max_y,
                                    int *min_y, int nType) {
...
static void osm_gps_map_print_tracks(OsmGpsMap *map) {
  ...
  if (priv->tracks || (priv->show_trip_history && priv->trip_history)) {
    ...
    if (priv->show_trip_history)
      osm_gps_map_print_track(priv, priv->trip_history, lw, &max_x, &min_x,
                              &max_y, &min_y, 1);
    GSList *tmp = priv->tracks;
    while (tmp != NULL) {
...
static gboolean osm_gps_map_purge_cache_check(G_GNUC_UNUSED gpointer key,
                                              gpointer value, gpointer user) {
  return (((OsmCachedTile *)value)->redraw_cycle !=
          ((OsmGpsMapPrivate *)user)->redraw_cycle);
}
...
void osm_gps_map_blit(OsmGpsMap *map, cairo_t *cr, cairo_operator_t op) {
  ...
  cairo_save(cr);
  cairo_translate(
      cr, -(1.5 * priv->map_factor - 1.) * priv->viewport_width * 0.5f,
...
static void osm_gps_map_init(OsmGpsMap *object) {
  ...
  priv->soup_session = soup_session_async_new_with_options(
      SOUP_SESSION_USER_AGENT, USER_AGENT, NULL);
  ...
  priv->tile_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                           (GDestroyNotify)cached_tile_free);
...
static void osm_gps_map_setup(OsmGpsMapPrivate *priv) {
  ...
  if ((priv->map_source == 0) || (strcmp(priv->repo_uri, uri) == 0)) {
    ...
    priv->null_tile =
        cairo_image_surface_create(CAIRO_FORMAT_ARGB32, TILESIZE, TILESIZE);
    ...
    if (uri) {
      ...
      priv->image_format =
          g_strdup(osm_gps_map_source_get_image_format(priv->map_source));
  ...
  priv->cache_dir =
      osm_gps_map_source_get_cache_dir(priv->map_source, priv->tile_dir, base);
...
static GObject *osm_gps_map_constructor(GType gtype, guint n_properties,
  ...
  GObject *object = G_OBJECT_CLASS(osm_gps_map_parent_class)
...
static void osm_gps_map_set_property(GObject *object, guint prop_id,
  ...
  case PROP_DOUBLE_PIXEL:
    ...
    if (!priv->idle_map_redraw)
      priv->idle_map_redraw =
          g_idle_add((GSourceFunc)osm_gps_map_idle_redraw, (gpointer)map);
    break;
  ...
  case PROP_PROXY_URI:
    if (g_value_get_string(value)) {
      ...
      g_object_set(G_OBJECT(priv->soup_session), SOUP_SESSION_PROXY_URI, uri,
                   NULL);
#else
      GValue val = {0, {{0}, {0}}};
      SoupURI *uri = soup_uri_new(priv->proxy_uri);
      ...
      g_object_set_property(G_OBJECT(priv->soup_session),
                            SOUP_SESSION_PROXY_URI, &val);
#endif
    } else
      priv->proxy_uri = NULL;
  ...
  case PROP_VIEWPORT_WIDTH:
    osm_gps_map_set_viewport(map, g_value_get_uint(value),
                             priv->viewport_height);
    break;
  case PROP_VIEWPORT_HEIGHT:
    osm_gps_map_set_viewport(map, priv->viewport_width,
...
static void osm_gps_map_get_property(GObject *object, guint prop_id,
...
void osm_gps_map_set_viewport(OsmGpsMap *map, guint width, guint height) {
  ...
  g_message("Set view port to %dx%d for source %d.", width, height,
            priv->map_source);
  ...
  priv->cr_surf = cairo_image_surface_create(
      CAIRO_FORMAT_ARGB32, 1.5 * priv->viewport_width + EXTRA_BORDER * 2,
...
static void osm_gps_map_class_init(OsmGpsMapClass *klass) {
  ...
  properties[PROP_DOUBLE_PIXEL] = g_param_spec_boolean(
      ...
      "double map pixels for better readability", FALSE,
      G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT);
  g_object_class_install_property(object_class, PROP_DOUBLE_PIXEL,
                                  properties[PROP_DOUBLE_PIXEL]);

  properties[PROP_AUTO_CENTER] = g_param_spec_boolean(
      "auto-center", "auto center", "map auto center", TRUE,
      G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT);
  g_object_class_install_property(object_class, PROP_AUTO_CENTER,
                                  properties[PROP_AUTO_CENTER]);

  g_object_class_install_property(
      object_class, PROP_RECORD_TRIP_HISTORY,
      g_param_spec_boolean(
          "record-trip-history", "record trip history",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean(
          "show-trip-history", "show trip history",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean(
          "auto-download", "auto download", "map auto download", TRUE,
          G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_REPO_URI,
      g_param_spec_string("repo-uri", "repo uri",
                          "map source tile repository uri", OSM_REPO_URI,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "proxy-uri", "proxy uri", "http proxy uri on NULL", NULL,
          G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_TILE_CACHE_DIR,
      g_param_spec_string("tile-cache", "tile cache",
                          "osm local tile cache dir", OSM_GPS_MAP_CACHE_AUTO,
                          G_PARAM_READABLE | G_PARAM_WRITABLE |
                              G_PARAM_CONSTRUCT_ONLY));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "tile-cache-base", "tile cache-base",
          "base directory to which friendly and auto paths are appended", NULL,
          G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_TILE_CACHE_DIR_IS_FULL_PATH,
      g_param_spec_boolean("tile-cache-is-full-path", "tile cache is full path",
                           "DEPRECIATED", FALSE,
                           G_PARAM_READABLE | G_PARAM_WRITABLE));

  properties[PROP_ZOOM] =
      g_param_spec_int("zoom", "zoom", "initial zoom level",
  ...
  properties[PROP_FACTOR] =
      g_param_spec_float("factor", "factor", "zooming adjustment factor", 0.4,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("max-zoom", "max zoom", "maximum zoom level",
                       MIN_ZOOM, /* minimum property value */
                       MAX_ZOOM, /* maximum property value */
                       OSM_MAX_ZOOM,
                       G_PARAM_READABLE | G_PARAM_WRITABLE |
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("min-zoom", "min zoom", "minimum zoom level",
                       MIN_ZOOM, /* minimum property value */
                       MAX_ZOOM, /* maximum property value */
                       OSM_MIN_ZOOM,
                       G_PARAM_READABLE | G_PARAM_WRITABLE |
  ...
  properties[PROP_LATITUDE] =
      g_param_spec_float("latitude", "latitude", "latitude in degrees",
  ...
  properties[PROP_LONGITUDE] =
      g_param_spec_float("longitude", "longitude", "longitude in degrees",
  ...
  properties[PROP_MAP_X] =
      g_param_spec_int("map-x", "map-x", "initial map x location",
                       G_MININT, /* minimum property value */
                       G_MAXINT, /* maximum property value */
                       890, G_PARAM_READABLE | G_PARAM_WRITABLE);
  g_object_class_install_property(object_class, PROP_MAP_X,
                                  properties[PROP_MAP_X]);

  properties[PROP_MAP_Y] =
      g_param_spec_int("map-y", "map-y", "initial map y location",
                       G_MININT, /* minimum property value */
                       G_MAXINT, /* maximum property value */
                       515, G_PARAM_READABLE | G_PARAM_WRITABLE);
  g_object_class_install_property(object_class, PROP_MAP_Y,
                                  properties[PROP_MAP_Y]);

  g_object_class_install_property(
      object_class, PROP_TILES_QUEUED,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int(
          "gps-track-width", "gps-track-width",
          "width of the lines drawn for the gps track",
          1,        /* minimum property value */
          G_MAXINT, /* maximum property value */
          4, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_GPS_POINT_R1,
      g_param_spec_int(
          "gps-track-point-radius", "gps-track-point-radius",
          "radius of the gps point inner circle",
          0,        /* minimum property value */
          G_MAXINT, /* maximum property value */
          5, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_GPS_POINT_R2,
      g_param_spec_int(
          "gps-track-highlight-radius", "gps-track-highlight-radius",
          "radius of the gps point highlight circle",
          0,        /* minimum property value */
          G_MAXINT, /* maximum property value */
          20, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  properties[PROP_MAP_SOURCE] = g_param_spec_uint(
      "map-source", "map source", "map source ID",
  ...
  properties[PROP_VIEWPORT_WIDTH] = g_param_spec_uint(
      "viewport-width", "Viewport width", "width of the viewable area",
      1,    /* minimum property value */
  ...
  properties[PROP_VIEWPORT_HEIGHT] = g_param_spec_uint(
      "viewport-height", "Viewport height", "height of the viewable area",
      1,    /* minimum property value */
      2048, /* maximum property value */
      1, G_PARAM_READABLE | G_PARAM_WRITABLE);
...
void osm_gps_map_download_maps(OsmGpsMap *map, coord_t *pt1, coord_t *pt2,
                               int zoom_start, int zoom_end) {
  ...
  if (pt1 && pt2) {
    ...
    for (zoom = zoom_start; zoom <= zoom_end; zoom++) {
      ...
      for (i = x1; i <= x2; i++) {
        ...
        for (j = y1; j <= y2; j++) {
          // x = i, y = j
          filename = g_strdup_printf("%s%c%d%c%d%c%d.%s", priv->cache_dir,
                                     G_DIR_SEPARATOR, zoom, G_DIR_SEPARATOR, i,
                                     G_DIR_SEPARATOR, j, priv->image_format);
...
static void _update_screen_pos(OsmGpsMap *map) {
  ...
  priv->map_x =
      lon2pixel(priv->map_zoom, priv->center_rlon) - priv->viewport_width / 2;
  priv->map_y =
      lat2pixel(priv->map_zoom, priv->center_rlat) - priv->viewport_height / 2;
...
void osm_gps_map_set_mapcenter(OsmGpsMap *map, float latitude, float longitude,
                               int zoom) {
  gboolean update;
...
void osm_gps_map_adjust_to(OsmGpsMap *map, coord_t *top_left,
                           coord_t *bottom_right) {
  gboolean update;
...
void osm_gps_map_auto_center_at(OsmGpsMap *map, float latitude,
                                float longitude) {
  OsmGpsMapPrivate *priv;
...
static void _on_track_dirty(G_GNUC_UNUSED MaepGeodata *track_state,
                            OsmGpsMap *map) {
  OsmGpsMapPrivate *priv;
...
void osm_gps_map_add_track(OsmGpsMap *map, MaepGeodata *track, int nId,
  ...
  st->iwp_prop =
      g_signal_connect(G_OBJECT(track), "notify::waypoint-highlight-index",
                       G_CALLBACK(_on_track_changed), (gpointer)map);
  st->dirty_sig = g_signal_connect(G_OBJECT(track), "dirty",
...
void osm_gps_map_add_image_with_alignment(OsmGpsMap *map, float latitude,
                                          float longitude,
                                          cairo_surface_t *image, float xalign,
                                          float yalign, const char *szName) {
  g_return_if_fail(OSM_IS_GPS_MAP(map));
...
void osm_gps_map_add_image(OsmGpsMap *map, float latitude, float longitude,
                           cairo_surface_t *image) {
  osm_gps_map_add_image_with_alignment(map, latitude, longitude, image, 0.5,
                                       0.5, 0);
}
...
void osm_gps_map_rename_image(OsmGpsMap *map, cairo_surface_t *image,
                              const char *sz) {
  OsmGpsMapPrivate *priv = map->priv;
  if (priv->images) {
    GSList *list;
...
void osm_gps_map_set_gps(OsmGpsMap *map, float latitude, float longitude,
                         float heading) {
  OsmGpsMapPrivate *priv;
...
coord_t osm_gps_map_get_co_ordinates(OsmGpsMap *map, int pixel_x, int pixel_y) {
  ...
  coord.rlat =
      pixel2lat(priv->map_zoom,
                priv->map_y + (pixel_y + (priv->map_factor - 1.f) *
                                             priv->viewport_height * 0.5f) /
                                  priv->map_factor);
  coord.rlon =
      pixel2lon(priv->map_zoom,
                priv->map_x + (pixel_x + (priv->map_factor - 1.f) *
                                             priv->viewport_width * 0.5f) /
                                  priv->map_factor);
  return coord;
}

void osm_gps_map_from_co_ordinates(OsmGpsMap *map, coord_t *coord, int *pixel_x,
                                   int *pixel_y) {
  OsmGpsMapPrivate *priv;
  ...
  if (pixel_x)
    *pixel_x = priv->map_factor *
                   (lon2pixel(priv->map_zoom, coord->rlon) - priv->map_x) -
               (priv->map_factor - 1.f) * priv->viewport_width * 0.5f;
  if (pixel_y)
    *pixel_y = priv->map_factor *
                   (lat2pixel(priv->map_zoom, coord->rlat) - priv->map_y) -
...
void osm_gps_map_screen_to_geographic(OsmGpsMap *map, gint pixel_x,
                                      gint pixel_y, gfloat *latitude,
                                      gfloat *longitude) {
  OsmGpsMapPrivate *priv;
  ...
  if (latitude)
    *latitude = rad2deg(
        pixel2lat(priv->map_zoom, priv->map_y + pixel_y - priv->drag_mouse_dy));
  if (longitude)
    *longitude = rad2deg(
        pixel2lon(priv->map_zoom, priv->map_x + pixel_x - priv->drag_mouse_dx));
}

void osm_gps_map_geographic_to_screen(OsmGpsMap *map, gfloat latitude,
                                      gfloat longitude, gint *pixel_x,
                                      gint *pixel_y) {
  OsmGpsMapPrivate *priv;
