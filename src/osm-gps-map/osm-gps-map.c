 ...
 */
static gchar* replace_string(const gchar* src,
                             const gchar* from,
                             const gchar* to);
...
static void osm_gps_map_tile_download_complete(SoupSession* session,
                                               ...
                                               gpointer user_data);
#endif
static void osm_gps_map_download_tile(OsmGpsMap* map,
                                      int zoom,
                                      int x,
                                      int y,
                                      gboolean redraw);
static void osm_gps_map_load_tile(OsmGpsMap* map,
                                  int zoom,
                                  int x,
                                  int y,
                                  int offset_x,
...
static void map_convert_coords_to_quadtree_string(gint x,
                                                  gint y,
                                                  gint zoomlevel,
                                                  gchar* buffer,
                                                  const gchar initial,
...
static gchar* get_tile_uri(const gchar* uri,
                           int uri_format,
                           int max_zoom,
                           int zoom,
                           int x,
  ...
  while (i < URI_FLAG_END) {
    ...
    switch (i & uri_format) {
      ...
      case URI_HAS_S:
        ...
        break;
      case URI_HAS_Q:
        map_convert_coords_to_quadtree_string(x, y, zoom, location, 't',
                                              "qrts");
        ...
        break;
      case URI_HAS_Q0:
        map_convert_coords_to_quadtree_string(x, y, zoom, location, '\0',
                                              "0123");
        s = g_strdup_printf("%s", location);
...
static void osm_gps_map_blit_surface(OsmGpsMap* map,
                                     cairo_surface_t* cr_surf,
                                     int offset_x,
                                     int offset_y,
                                     int modulo,
...
gchar* get_cached_file(const gchar* cache_dir,
                       const gchar* format,
                       int zoom,
                       int x,
                       int y) {
...
static OsmCachedTile* osm_gps_map_find_bigger_tile(OsmGpsMap* map,
                                                   int zoom,
                                                   int x,
                                                   int y,
                                                   int* zoom_found) {
...
static OsmCachedTile* osm_gps_map_render_missing_tile_upscaled(OsmGpsMap* map,
                                                               int zoom,
                                                               int x,
                                                               int y,
                                                               int* modulo,
...
static OsmCachedTile* osm_gps_map_render_missing_tile(OsmGpsMap* map,
                                                      int zoom,
                                                      int x,
                                                      int y,
                                                      int* modulo,
...
static void osm_gps_map_print_track(OsmGpsMapPrivate* priv,
                                    MaepGeodata* track,
                                    int lw,
                                    int* max_x,
                                    int* min_x,
...
static GObject* osm_gps_map_constructor(GType gtype,
                                        guint n_properties,
...
static void osm_gps_map_set_property(GObject* object,
                                     guint prop_id,
                                     const GValue* value,
                                     GParamSpec* pspec) {
  ...
  switch (prop_id) {
    ...
    case PROP_MAP_SOURCE: {
      ...
      if (priv->map_source != old &&
          priv->map_source < OSM_GPS_MAP_SOURCE_LAST && priv->repo_uri) {
...
static void osm_gps_map_get_property(GObject* object,
                                     guint prop_id,
                                     GValue* value,
                                     GParamSpec* pspec) {
  g_return_if_fail(OSM_IS_GPS_MAP(object));
...
static void osm_gps_map_class_init(OsmGpsMapClass* klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "repo-uri", "repo uri", "map source tile repository uri",
          OSM_REPO_URI,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "tile-cache", "tile cache", "osm local tile cache dir",
  ...
  *url = NULL;
  switch (source) {
    ...
    case OSM_GPS_MAP_SOURCE_VIRTUAL_EARTH_HYBRID:
      ...
      *url =
          "http://windows.microsoft.com:80/en-gb/windows-live/"
...
void osm_gps_map_adjust_to(OsmGpsMap* map,
                           coord_t* top_left,
...
void osm_gps_map_add_image_with_alignment(OsmGpsMap* map,
                                          float latitude,
                                          float longitude,
...
void osm_gps_map_rename_image(OsmGpsMap* map,
                              cairo_surface_t* image,
                              const char* sz) {
  OsmGpsMapPrivate* priv = map->priv;
  if (priv->images) {
...
void osm_gps_map_set_gps(OsmGpsMap* map,
                         float latitude,
                         float longitude,
                         float heading) {
  OsmGpsMapPrivate* priv;
...
coord_t osm_gps_map_get_co_ordinates(OsmGpsMap* map, int pixel_x, int pixel_y) {
  ...
  coord.rlat = pixel2lat(
      priv->map_zoom,
      priv->map_y +
          (pixel_y + (priv->map_factor - 1.f) * priv->viewport_height * 0.5f) /
              priv->map_factor);
  coord.rlon = pixel2lon(
      priv->map_zoom,
      priv->map_x +
          (pixel_x + (priv->map_factor - 1.f) * priv->viewport_width * 0.5f) /
              priv->map_factor);
  return coord;
}

void osm_gps_map_from_co_ordinates(OsmGpsMap* map,
                                   coord_t* coord,
                                   int* pixel_x,
                                   int* pixel_y) {
  OsmGpsMapPrivate* priv;
