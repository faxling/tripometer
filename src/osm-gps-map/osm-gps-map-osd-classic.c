...
static OsmGpsMapSource_t map_sources[] = {
    OSM_GPS_MAP_SOURCE_OPENSTREETMAP,          OSM_GPS_MAP_SOURCE_MML_PERUSKARTTA,
    OSM_GPS_MAP_SOURCE_MML_ORTOKUVA,           OSM_GPS_MAP_SOURCE_MML_TAUSTAKARTTA,
    OSM_GPS_MAP_SOURCE_OPENSTREETMAP_RENDERER, OSM_GPS_MAP_SOURCE_OPENCYCLEMAP,
    OSM_GPS_MAP_SOURCE_OSM_PUBLIC_TRANSPORT,   OSM_GPS_MAP_SOURCE_GOOGLE_STREET,
    OSM_GPS_MAP_SOURCE_VIRTUAL_EARTH_STREET,   OSM_GPS_MAP_SOURCE_VIRTUAL_EARTH_SATELLITE,
...
/* for the shadow */
static void osm_gps_map_draw_balloon_shape(cairo_t* cr, int x0, int y0, int x1, int y1,
                                           gboolean bottom, int px, int py, int px0, int px1)
{

  cairo_move_to(cr, x0, y0 + BALLOON_CORNER_RADIUS);
  cairo_arc(cr, x0 + BALLOON_CORNER_RADIUS, y0 + BALLOON_CORNER_RADIUS, BALLOON_CORNER_RADIUS,
            -M_PI, -M_PI / 2);
  if (!bottom)
  {
    /* insert top pointer */
  ...
  cairo_arc(cr, x1 - BALLOON_CORNER_RADIUS, y0 + BALLOON_CORNER_RADIUS, BALLOON_CORNER_RADIUS,
            -M_PI / 2, 0);
  cairo_line_to(cr, x1, y1 - BALLOON_CORNER_RADIUS);
  cairo_arc(cr, x1 - BALLOON_CORNER_RADIUS, y1 - BALLOON_CORNER_RADIUS, BALLOON_CORNER_RADIUS, 0,
            M_PI / 2);
  if (bottom)
  {
  ...
  cairo_arc(cr, x0 + BALLOON_CORNER_RADIUS, y1 - BALLOON_CORNER_RADIUS, BALLOON_CORNER_RADIUS,
            M_PI / 2, M_PI);

  cairo_close_path(cr);
}

static void osd_render_balloon(osm_gps_map_osd_t* osd)
{
  ...
  /* get zoom */
  gint zoom;
  g_object_get(OSM_GPS_MAP(osd->map), "zoom", &zoom, "viewport-width", &width, "viewport-height",
               &height, NULL);

  /* ------- convert given coordinate into screen position --------- */
  gint xs, ys;
  osm_gps_map_geographic_to_screen(OSM_GPS_MAP(osd->map), priv->balloon.lat, priv->balloon.lon, &xs,
                                   &ys);
  ...
  /* --------- draw shadow --------------- */
  osm_gps_map_draw_balloon_shape(cr, x0 + BALLOON_SHADOW, y0 + BALLOON_SHADOW, x1 + BALLOON_SHADOW,
                                 y1 + BALLOON_SHADOW, bottom, pointer_x, pointer_y,
  ...
  /* --------- draw main shape ----------- */
  osm_gps_map_draw_balloon_shape(cr, x0, y0, x1, y1, bottom, pointer_x, pointer_y, pointer_x0,
                                 pointer_x1);
  ...
  if (priv->balloon.cb)
  {
    ...
    cairo_rectangle(cr, priv->balloon.rect.x, priv->balloon.rect.y, priv->balloon.rect.w,
                    priv->balloon.rect.h);
    cairo_clip(cr);
...
static gboolean osd_balloon_check(osm_gps_map_osd_t* osd, gboolean click, gint state, gint x,
                                  gint y)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  ...
  gboolean is_in =
      (x > xs) && (x < xs + priv->balloon.rect.w) && (y > ys) && (y < ys + priv->balloon.rect.h);
...
void osm_gps_map_osd_draw_balloon(osm_gps_map_osd_t* osd, float latitude, float longitude,
                                  OsmGpsMapBalloonCallback cb, gpointer data)
{
  ...
  priv->balloon.surface =
      cairo_image_surface_create(CAIRO_FORMAT_ARGB32, BALLOON_W + 2, BALLOON_H + 2);
...
static void osd_zoom_shape(cairo_t* cr, gint x, gint y)
{
...
static void osd_labels(cairo_t* cr, gint width, gboolean enabled, double fg[3], double disabled[3])
{
  if (enabled)
    cairo_set_source_rgb(cr, fg[0], fg[1], fg[2]);
  else
    cairo_set_source_rgb(cr, disabled[0], disabled[1], disabled[2]);
  cairo_set_line_width(cr, width);
}
#else
static void osd_labels(cairo_t* cr, gint width, gboolean enabled)
{
  if (enabled)
    cairo_set_source_rgb(cr, OSD_COLOR);
  else
    cairo_set_source_rgb(cr, OSD_COLOR_DISABLED);
  cairo_set_line_width(cr, width);
}
#endif

#ifdef OSD_SHADOW_ENABLE
static void osd_labels_shadow(cairo_t* cr, gint width, gboolean enabled)
{
...
static void osd_dpad_shape(cairo_t* cr, gint x, gint y)
{
...
#ifdef OSD_SHADOW_ENABLE
static void osd_shape_shadow(cairo_t* cr)
{
...
#ifndef OSD_COLOR
/* if no color has been specified we just use the default colors */
static void osd_shape(cairo_t* cr, double bg[3], double fg[3])
{
...
#else
static void osd_shape(cairo_t* cr)
{
...
static osd_button_t osd_check_dpad(gint x, gint y)
{
...
static osd_button_t osd_check_zoom(gint x, gint y)
{
...
static void osd_source_shape(osd_priv_t* priv, cairo_t* cr, gint x, gint y)
{
...
static void osd_source_content(osm_gps_map_osd_t* osd, cairo_t* cr, gint offset)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  ...
  else
  {
    ...
    if (offset == 1)
    {
      ...
      for (i = 0; i < num_map_sources + OSD_DPIX_EXTRA; i++)
      {
        ...
        {
          cairo_rectangle(cr, x - OSD_TEXT_BORDER / 2, y - OSD_TEXT_SKIP,
                          priv->source_sel.width - OSD_TEXT_BORDER, step + OSD_TEXT_SKIP);
          cairo_fill(cr);
...
static void osd_render_source_sel(osm_gps_map_osd_t* osd, gboolean force_rerender)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;

  if (!priv->source_sel.surface || (priv->source_sel.rendered && !force_rerender))
    return;
...
static void osd_source_reallocate(osm_gps_map_osd_t* osd)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  ...
  if (priv->source_sel.expanded)
  {
    ...
    for (i = 0; i < num_map_sources; i++)
    {
      ...
      if (extents.width > max_w)
        max_w = extents.width;
      if (extents.height > priv->source_sel.max_h)
        priv->source_sel.max_h = extents.height;
    }
    cairo_destroy(cr);

    priv->source_sel.width = max_w + 2 * OSD_TEXT_BORDER;
    priv->source_sel.height =
        (num_map_sources + OSD_DPIX_EXTRA) * (priv->source_sel.max_h + 2 * OSD_TEXT_SKIP) +
        2 * OSD_TEXT_BORDER + OSD_DPIX_SKIP;
  ...
  priv->source_sel.surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, w + 2, h + 2);
...
static void osd_source_toggle(osm_gps_map_osd_t* osd)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  guint width;
  ...
  priv->source_sel.handler_id = g_timeout_add(OSD_TIME / OSD_HZ, osd_source_animate, osd);
}

/* check if the user clicked inside the source selection area */
static osd_button_t osd_source_check(osm_gps_map_osd_t* osd, gint state, gint x, gint y)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  guint width, height;
  ...
  if (priv->source_sel.expanded && !priv->source_sel.handler_id)
  {
    ...
    if (x > OSD_S_PW && x < OSD_S_PW + OSD_S_EXP_W && y > 0 && y < OSD_S_EXP_H)
    {
      ...
      if (state == OSD_STATE_DOWN)
      {
        ...
        if (py >= 0 && py < num_map_sources && old != map_sources[py])
        {
        ...
        {
          y -= num_map_sources * (priv->source_sel.max_h + 2 * OSD_TEXT_SKIP) + OSD_TEXT_SKIP +
               OSD_DPIX_SKIP;
          if (y >= 0)
          {
            gboolean dpix = 0;
...
#ifndef OSD_NO_DPAD
static void osd_dpad_labels(cairo_t* cr, gint x, gint y)
{
  /* move reference to dpad center */
  x += D_RAD;
  y += D_RAD;

  const static gint offset[][3][2] = {/* left arrow/triangle */
...
static void osd_dpad_gps(cairo_t* cr, gint x, gint y)
{
  /* move reference to dpad center */
...
#ifdef OSD_CONTROLS
static void osd_zoom_labels(cairo_t* cr, gint x, gint y)
{
...
{
  float aob = acos(cos(lat1) * cos(lat2) * cos(lon2 - lon1) + sin(lat1) * sin(lat2));

  return (aob * 6371000.0); /* great circle radius in meters */
}

static void osd_render_nav(osm_gps_map_osd_t* osd)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
...
#define ARROW_LENGTH 0.7
  ...
  if (pos)
  {
    float arot = get_bearing(pos->rlat, pos->rlon, deg2rad(priv->nav.lat), deg2rad(priv->nav.lon));

    cairo_move_to(cr, x + radius * ARROW_LENGTH * sin(arot),
                  y + radius * ARROW_LENGTH * -cos(arot));

    cairo_line_to(cr, x + radius * -ARROW_LENGTH * sin(arot + ARROW_WIDTH),
                  y + radius * -ARROW_LENGTH * -cos(arot + ARROW_WIDTH));

    cairo_line_to(cr, x + radius * -0.5 * ARROW_LENGTH * sin(arot),
    ...
    float dist = get_distance(pos->rlat, pos->rlon, deg2rad(priv->nav.lat), deg2rad(priv->nav.lon));
...
/* check if the user clicked inside the source selection area */
static osd_button_t osd_nav_check(osm_gps_map_osd_t* osd, gint state, gint x, gint y)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  guint width, height;
  ...
  if (x >= 0 && y >= 0 && x <= OSD_NAV_W && y <= OSD_NAV_H)
  {
    if (y < priv->nav.click_sep)
      osm_gps_map_set_center(OSM_GPS_MAP(osd->map), priv->nav.lat, priv->nav.lon);
    else
    {
...
void osm_gps_map_osd_clear_nav(osm_gps_map_osd_t* osd)
{
  ...
  if (priv->nav.surface)
  {
    ...
    if (priv->nav.name)
      g_free(priv->nav.name);
  }
  osd->render(osd);
}

void osm_gps_map_osd_draw_nav(osm_gps_map_osd_t* osd, gboolean imperial, float latitude,
                              float longitude, char* name)
{
  g_return_if_fail(osd);
  ...
  /* allocate balloon surface */
  priv->nav.surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, OSD_NAV_W + 2, OSD_NAV_H + 2);
...
static void osd_render_heart_shape(cairo_t* cr, gint x, gint y, gint s)
{
...
static void osd_render_heart(cairo_t* cr, gint x, gint y, gint s, gboolean ok)
{
  /* xyz */
...
static void osd_render_hr(osm_gps_map_osd_t* osd)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
  ...
  osd_render_heart(cr, OSD_HR_W / 2 - extents.width / 2 - OSD_HR_FONT_SIZE / 8, OSD_HR_H / 2,
                   OSD_HR_FONT_SIZE / 5, priv->hr.ok);
...
void osm_gps_map_osd_draw_hr(osm_gps_map_osd_t* osd, gboolean ok, gint rate)
{
  ...
  if (rate != OSD_HR_NONE && !priv->hr.surface)
    priv->hr.surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, OSD_HR_W, OSD_HR_H);
...
static void osd_render_crosshair(osm_gps_map_osd_t* osd)
{
  ...
  cairo_set_source_rgba(cr, 0, 0xf4 / 255.0, 0xfb / 255.0, 1);
...
#ifdef OSD_CONTROLS
static void osd_render_controls(osm_gps_map_osd_t* osd)
{
  osd_priv_t* priv = (osd_priv_t*)osd->priv;
...
static void osd_draw(osm_gps_map_osd_t* osd, cairo_t* cr)
{
  ...
  if (!priv->source_sel.surface)
  {
    ...
    priv->source_sel.surface =
        cairo_image_surface_create(CAIRO_FORMAT_ARGB32, OSD_S_W + 2, OSD_S_H + 2);
  ...
  if (priv->balloon.surface)
  {
    ...
    gint x, y;
    osm_gps_map_geographic_to_screen(OSM_GPS_MAP(osd->map), priv->balloon.lat, priv->balloon.lon,
                                     &x, &y);

    /* check if balloon needs to be rerendered */
    osd_render_balloon(osd);

    cairo_set_source_surface(cr, priv->balloon.surface, x + priv->balloon.offset_x,
