...
QString Maep::GeonamesPlace::coordinateToString(
    QGeoCoordinate::CoordinateFormat format) const {
  return m_coordinate.toString(format);
}
QString Maep::GeonamesEntry::coordinateToString(
    QGeoCoordinate::CoordinateFormat format) const {
  return m_coordinate.toString(format);
}
...
#define NAN (0.0 / 0.0)
  ...
  emit characteristicsChanged(
      (qreal)maep_geodata_track_get_metric_length(track),
...
bool Maep::Track::setMetricAccuracy(qreal value) {
  ...
  ret = maep_geodata_track_set_metric_accuracy(
      track, (value <= 0) ? G_MAXFLOAT : (gfloat)value);
  if (ret) {
...
void Maep::Track::addWayPoint(const QGeoCoordinate& coord,
                              const QString& name,
                              const QString& comment,
                              const QString& description) {
  maep_geodata_add_waypoint(
      track, coord.latitude(), coord.longitude(), name.toLocal8Bit().data(),
...
static void osm_gps_map_qt_coordinate(Maep::GpsMap* widget,
                                      GParamSpec* pspec,
                                      OsmGpsMap* map);
...
Maep::GpsMap::GpsMap(QQuickItem* parent)
  ...
  g_pTheMap = this;
  gint source =
      gconf_get_int(GCONF_KEY_SOURCE, OSM_GPS_MAP_SOURCE_OPENSTREETMAP);
  gint overlaySource =
      gconf_get_int(GCONF_KEY_OVERLAY_SOURCE, OSM_GPS_MAP_SOURCE_NULL);
 ...
 // bool wikipedia = FALSE;  // gconf_get_bool(GCONF_KEY_WIKIPEDIA, FALSE);
  ...
  map = OSM_GPS_MAP(
      g_object_new(OSM_TYPE_GPS_MAP, "map-source", source, "tile-cache",
                   OSM_GPS_MAP_CACHE_FRIENDLY, "tile-cache-base", path,
                   "auto-center", FALSE, "record-trip-history", FALSE,
                   "show-trip-history", FALSE, "gps-track-point-radius", 10,
  ...
  */
  ...
  connect(&compass, SIGNAL(readingChanged()), this,
...
Maep::GpsMap::~GpsMap() {
  ...
  g_object_get(map, "zoom", &zoom, "map-source", &source, "latitude", &lat,
               "longitude", &lon, "double-pixel", &dpix, NULL);
...
void Maep::GpsMap::ensureOverlay(Source source) {
  ...
  overlay = OSM_GPS_MAP(g_object_new(
      OSM_TYPE_GPS_MAP, "map-source", (guint)source, "tile-cache",
  ...
  g_object_bind_property(
      G_OBJECT(map), "zoom", G_OBJECT(overlay), "zoom",
      (GBindingFlags)(G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE));
  g_object_bind_property(
      G_OBJECT(map), "factor", G_OBJECT(overlay), "factor",
      (GBindingFlags)(G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE));
  g_object_bind_property(
      G_OBJECT(map), "double-pixel", G_OBJECT(overlay), "double-pixel",
      (GBindingFlags)(G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE));
  g_object_bind_property(
      G_OBJECT(map), "viewport-width", G_OBJECT(overlay), "viewport-width",
      (GBindingFlags)(G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE));
  g_object_bind_property(
      G_OBJECT(map), "viewport-height", G_OBJECT(overlay), "viewport-height",
      (GBindingFlags)(G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE));
  /* Workaround to bind lat and lon together. */
  g_signal_connect_object(G_OBJECT(map), "notify::latitude",
                          G_CALLBACK(onLatLon), (gpointer)overlay,
                          (GConnectFlags)0);
...
bool Maep::GpsMap::mapSized() {
  ...
  if (!surf) {
    ...
    img = new QImage(
        cairo_image_surface_get_data(surf), cairo_image_surface_get_width(surf),
        cairo_image_surface_get_height(surf), QImage::Format_ARGB32);
  ...
  switch (touchEvent->type()) {
    ...
    case QEvent::TouchUpdate: {
      ...
      if (dragging) {
        ...
        osm_gps_map_uppdate_offset(map, delta.x() - nLastDeltaX,
                                   delta.y() - nLastDeltaY);
        nLastDeltaX = delta.x();
        nLastDeltaY = delta.y();
        osm_gps_map_scroll(map);
...
void Maep::GpsMap::loadTrack(const QString& sTrackName, int nId) {
  ...
  if (t.nType == 0 || t.nType == 2) {
    ...
    GError* error = 0;
    MaepGeodata* track =
        maep_geodata_new_from_file(sGpxFileName.toUtf8().data(), &error);
    ...
    if (m_ocMarkers.contains(nId) == false) {
      ...
      cairo_surface_t* pSurface =
          cairo_image_surface_create_from_png(szSymName);
...
void Maep::GpsMap::saveMark(int nId) {
  ...
  QMetaObject::invokeMethod(g_pTheTrackModel, "trackAdd",
                            Q_ARG(QString, sTrackName));
}
...
void Maep::GpsMap::saveTrack(G_GNUC_UNUSED int nId) {
  ...
  maep_geodata_to_file(track_current->get(), sGpxFileName.toLatin1().data(),
                       &error);
...
void Maep::GpsMap::setLookAt(float lat, float lon) {
  ...
  // QGeoPositionInfo info = QGeoPositionInfo(coord,
...
void Maep::GpsMap::positionUpdate(const QGeoPositionInfo& info) {
  ...
  // g_message("position is %f %f, heading %g, h_acc %g",
...
void Maep::GpsMap::setTrack(Maep::Track* track) {
  ...
  if (track) {
    ...
    if (!track->getPath().isEmpty())
      gconf_set_string(GCONF_KEY_TRACK_PATH,
                       track->getPath().toLocal8Bit().data());
...
QString Maep::GpsMap::getCenteredTile(Maep::GpsMap::Source source) const {
  ...
  osm_gps_map_get_tile_xy_at(map, coordinate.latitude(), coordinate.longitude(),
                             &zoom, &x, &y);
  cache_dir = osm_gps_map_source_get_cache_dir(
      (OsmGpsMapSource_t)source, OSM_GPS_MAP_CACHE_FRIENDLY, base);
  g_free(base);
...
void Maep::GpsMapCover::setMap(Maep::GpsMap* map) {
  map_ = map;
  QObject::connect(map, &Maep::GpsMap::mapChanged, this,
                   &Maep::GpsMapCover::updateCover);
  emit mapChanged();
}
