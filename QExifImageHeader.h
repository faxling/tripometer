...
typedef QVector< QExifURational > QExifURV;


struct PosValDeg : public QExifURV
{
  const static unsigned int PREC = 1000000;
  PosValDeg(double v) : QExifURV({ { (unsigned int)abs(v*PREC), PREC }, { 0, 1 }, { 0, 1 } })
  {
...
public:
  ...
  QExifValue(const QExifURV &value);
  ...
  double toLng(const QExifValue &tRef) const;
  double toLat(const QExifValue &tRef) const;
  ...
  QExifURV toRationalVector() const;
...
public:
  ...
  QExifValue operator[] (ImageTag tag) const;
  QExifValue operator[] (ExifExtendedTag tag) const;
  QExifValue operator[] (GpsTag tag) const;
  ...
  void setLngWithRef(GpsTag tag, double value , GpsTag refTag);
  void setLatWithRef(GpsTag tag, double value , GpsTag refTag);
